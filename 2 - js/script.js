...
function firstExercise(employees) {
    ...
    for (let i = 0; i < employees.length; i++) {
        if (employees[i].position === "jr. QA engineer") {
            result.push(employees[i].surname, employees[i].name, employees[i].salary, employees[i].department);
        }
    }
  
    return result.sort(function(a, b) {
        
        return b.salary-a.salary  //сортировка по уменьшению зп
    })
